...
fn search<const PV: bool>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    let mut improved_val=0;
    
    if !in_check && td.ply >= 2 && td.stack[td.ply - 2].static_eval != Score::NONE {
        improved_val = static_eval - td.stack[td.ply - 2].static_eval
    }
    let improving = improved_val > 0;
    ...
    // Razoring
    if !PV && !in_check && eval < alpha - 300 - 250 * depth * depth - depth * improved_val/9 {
